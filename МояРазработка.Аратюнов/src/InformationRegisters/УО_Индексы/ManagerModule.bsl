...
Процедура Сдвинуть(Набор, Шаг) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Цель = Движение(Выборка);
		Цель.Индекс = Выборка.Индекс - Шаг;
		Цель.Получить();
		
		Если ЗначениеЗаполнено(Цель.Набор) Тогда
		
			ИсходнаяЗапись = Движение(Выборка);
			ИсходнаяЗапись.Набор = Цель.Набор;
			Цель.Набор = Набор;
			Цель.Запись();
			ИсходнаяЗапись.Записать();
